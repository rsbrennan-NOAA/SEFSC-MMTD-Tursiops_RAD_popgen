#!/usr/bin/env python3
"""
Moments demographic inference for Tursiops populations - Model 04
"""
...
# Generate random migration rates (log-uniform distribution for better coverage)
migration_rates = []
migration_names = [
    "m_CGulf_to_Int", "m_Int_to_CGulf", "m_offshore_to_int", "m_int_to_offshore",
    "m_offshore_to_CGulf", "m_CGulf_to_offshore", "m_offshore_to_B", "m_B_to_offshore",
    "m_intermediate_to_coastal", "m_coastal_to_intermediate"
]

print(f"\nMigration rates chosen:")
for i, name in enumerate(migration_names):
    # Log-uniform between 1e-10 and 0.1
    rate = np.random.uniform((1e-7), (1e-3))
...
# Update the YAML with random starting values for Model 05
