...
params.pop_file = "${HOME}/Tursiops-RAD-popgen/analysis/pop_structure/fourpop_all_noHybs.clust"
params.proj = "224,64,66,66"
...
process GENBOOTSTRAP {
    ...
    output:
    tuple val(bootstrap_id), path("bootstrap_${bootstrap_id}.vcf")
...
process EASYSFS {
    ...
    # Run easySFS
    easySFS.py -i ${bootstrap_vcf} -p ${pop_file} \
        --order ${params.pop_order} --proj ${params.proj} --outdir .
    """
}
workflow {
    ...
    processvcf_output = PROCESSVCF(vcf_ch, genome_ch)
    header_ch = processvcf_output[0]
    chunks_ch = processvcf_output[1].flatten().filter { it.toString().endsWith('.vcf') }.collect()
    
    // Generate bootstrap replicates
    bootstrapped_vcfs = GENBOOTSTRAP(chunks_ch, header_ch, bootstrap_ids_ch)
    
    // Run easySFS on each bootstrap replicate
    EASYSFS(bootstrapped_vcfs, pop_ch)
}
