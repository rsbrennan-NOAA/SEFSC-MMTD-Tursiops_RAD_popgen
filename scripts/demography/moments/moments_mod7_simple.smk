...
rule all:
    input:
        f"{ANALYSIS_DIR}/mod7_simple_{RUN_NAME}"

# Round 1: 80 reps with random starting values
rule round1:
    output:
        f"{ANALYSIS_DIR}/mod7_simple/output_yaml/model_{MODEL_NUMBER}_rep{{rep}}_{METHOD}_downSample.yaml"
    log:
        f"{ANALYSIS_DIR}/mod7_simple/logs/round1_model_{MODEL_NUMBER}_rep{{rep}}.log"
    name: "mts_r1"
    params:
        rep = "{rep}",
        model = MODEL_NUMBER,
        script = f"{SCRIPT_DIR}/moments_mod7_simple_round-01.py",
    conda:
        "moments"
    resources:
        mem_mb = 6000,
...
rule round2:
    input:
        expand(f"{ANALYSIS_DIR}/mod7_simple/output_yaml/model_{MODEL_NUMBER}_rep{{r1_rep}}_{METHOD}_downSample.yaml",
               r1_rep=ROUND1_REPS)
    output:
        f"{ANALYSIS_DIR}/mod7_simple/round2/output_yaml/model_{MODEL_NUMBER}_rep{{rep}}_{METHOD}_downSample-Round2.yaml"
    name: "mts_r2"
    log:
        f"{ANALYSIS_DIR}/mod7_simple/logs/round2_model_{MODEL_NUMBER}_rep{{rep}}.log"
    params:
        rep = "{rep}",
        model = MODEL_NUMBER,
        script = f"{SCRIPT_DIR}/moments_mod7_simple_round-02.py",
    conda:
        "moments"
    resources:
        mem_mb = 8000,
...
rule round3:
    input:
        expand(f"{ANALYSIS_DIR}/mod7_simple/round2/output_yaml/model_{MODEL_NUMBER}_rep{{r2_rep}}_{METHOD}_downSample-Round2.yaml",
               r2_rep=ROUND2_REPS)
    output:
        f"{ANALYSIS_DIR}/mod7_simple/round3/output_yaml/model_{MODEL_NUMBER}_rep{{rep}}_{METHOD}_downSample-Round3.yaml"
    name: "mts_r3"
    log:
        f"{ANALYSIS_DIR}/mod7_simple/logs/round3_model_{MODEL_NUMBER}_rep{{rep}}.log"
    conda:
        "moments"  # environment name
    params:
        rep = "{rep}",
        model = MODEL_NUMBER,
        script = f"{SCRIPT_DIR}/moments_mod7_simple_round-03_plus.py",
    resources:
        mem_mb = 8000,
        runtime = "24h",
        cpus_per_task = 1,
...
rule round4:
    input:
        expand(f"{ANALYSIS_DIR}/mod7_simple/round3/output_yaml/model_{MODEL_NUMBER}_rep{{r3_rep}}_{METHOD}_downSample-Round3.yaml",
               r3_rep=ROUND3_REPS)
    output:
        f"{ANALYSIS_DIR}/mod7_simple/round4/output_yaml/model_{MODEL_NUMBER}_rep{{rep}}_{METHOD}_downSample-Round4.yaml"
    name: "mts_r4"
    log:
        f"{ANALYSIS_DIR}/mod7_simple/logs/round4_model_{MODEL_NUMBER}_rep{{rep}}.log"
    conda:
        "moments"  
    params:
        rep = "{rep}",
...
rule rename_final:
    input:
        expand(f"{ANALYSIS_DIR}/mod7_simple/round4/output_yaml/model_{MODEL_NUMBER}_rep{{rep}}_{METHOD}_downSample-Round4.yaml",
               rep=ROUND4_REPS)
    output:
