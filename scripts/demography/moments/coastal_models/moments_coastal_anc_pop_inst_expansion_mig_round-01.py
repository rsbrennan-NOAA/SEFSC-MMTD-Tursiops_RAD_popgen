...
n_ancestor = int(np.random.uniform(500, 40000))          # N_Ancestor
n_bottleneck = int(np.random.uniform(50, 5000))          # N_bottleneck
n_catlantic_start = int(np.random.uniform(50, 5000))     # N_CAtlantic_start
n_cgulf_start = int(np.random.uniform(50, 5000))         # N_CGulf_start
n_catlantic_end = int(np.random.uniform(100, 40000))     # N_CAtlantic_end
n_cgulf_end = int(np.random.uniform(100, 40000))         # N_CGulf_end
...
for deme in yaml_data['demes']:
    if deme['name'] == 'Ancestor':
        ...
        # Epoch 0: Constant size after split until instant expansion
        deme['epochs'][0]['end_time'] = expansion_start_val
        deme['epochs'][0]['start_size'] = n_catlantic_start
        # Epoch 1: Instant expansion to final size
        deme['epochs'][1]['start_size'] = n_catlantic_end  
    elif deme['name'] == 'CGulf':
        # Epoch 0: Constant size after split until instant expansion
        deme['epochs'][0]['end_time'] = expansion_start_val 
        deme['epochs'][0]['start_size'] = n_cgulf_start
        # Epoch 1: Instant expansion to final size
        deme['epochs'][1]['start_size'] = n_cgulf_end 
