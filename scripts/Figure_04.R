...
# ---------------------------------------------------------------------------------
 ...
 nonsig2 <-(o$gradient[,3] >= 1 )
 nonsigCenter1 <- o$center[,2] <= .5
nonsigCenter2 <- o$center[,3] >= .5
   
   nonsig_all <- !nonsig | !nonsig2 | !nonsigCenter1| !nonsigCenter2
 sum(nonsig_all)
 nonsig <- !nonsig_all
 ...
 # drop the shallow points- we don't care about these for the paper:
 
 plot_data2 <- subset(plot_data, type != "shallow")
 plot_data <- plot_data2
 
 # Create the plot
 p <- ggplot() +
   # Plot the curves by type with  colors and line widths
 ...
 p2 <- ggplot() +
   ...
   scale_fill_manual(
     name = NULL,
     values = c("Neutral" = "grey50", "Steep" = "firebrick3"),
     guide = guide_legend(
 ...
 ggsave(filename="figures/manuscript/species_maintenance.pdf",
        final_plot,
        height=5, width=7)
 ggsave(filename="figures/manuscript/species_maintenance.png",
        final_plot,
        height=5, width=7)
 ...
 ## add in shallow for supplemental figure.
...
#        h=6.5, w=7)
 ...
 plt1 <- ggarrange(depth_plot, p, common.legend=FALSE, nrow=1, labels="AUTO",
                   widths=c(1.7, 1))
 plt1
 final_plot <- ggarrange(p, p2, widths=c(1, 3), common.legend=TRUE, 
                         nrow=1, labels="AUTO")
 final_plot
 
 # Save the final combined plot
 ggsave(filename="figures/species_maintenance_shallow.pdf",
        final_plot,
        height=4, width=10)
 ggsave(filename="figures/species_maintenance_shallow.png",
        final_plot,
        height=4, width=10)
